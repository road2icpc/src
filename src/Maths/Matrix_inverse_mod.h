//# Title: Matrix inverse mod prime
//# Description: Yoinked from kactl. Returns rank; result is stored in $A$ unless singular (rank $< n$). For prime powers, repeatedly set $A^{-1} = A^{-1} (2I - AA^{-1})\  (\text{mod }p^k)$ where $A^{-1}$ starts as the inverse of A mod p, and k is doubled in each step.
