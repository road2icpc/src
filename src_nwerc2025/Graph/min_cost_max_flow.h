...
struct MCMF {
	...
	void addEdge(int from, int to, ll cap, ll cost) {
		if (from == to) return;
		ed[from].push_back(edge{ from,to,ed[to].size(),cap,cost,0 });
		ed[to].push_back(edge{ to,from,ed[from].size()-1,0,-cost,0 });
	}
