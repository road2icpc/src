...
//# Description: [kactl] Finds the argument minimizing the function $f$ in the interval $[a,b]$ assuming $f$ is unimodal on the interval, i.e. has only one local minimum and no local maximum. The maximum error in the result is $eps$. Works equally well for maximization with a small change in the code. See TernarySearch.h in the Various chapter for a discrete version.
//# Usage:
//#  \texttt{double func(double x) \{ return 4+x+.3*x*x; \}}\\
//#  \texttt{double xmin = gss(-1000,1000,func);}
//# Complexity: $\mathcal{O}(\log((b - a) / \epsilon))$
